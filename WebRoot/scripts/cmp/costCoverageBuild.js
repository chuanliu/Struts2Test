(function() {
	var space = Ext.namespace('Digicompass.web.cmp.cost.costCoverageBuild');
	
	space.refresh = function(arg,planned) {
		var id = 'COST_COVERAGE_BUILD_GRID_' + planned + '_' + (arg.scenarioId||'')+(arg.versionId||'');;
		...
		var title = 'New Coverage Build[Planned]';
		var moduleType = 'MOD_COST_COVERAGE_PLANNED_BUILD';
		var tableName = 'TB_STG_COST_COVERAGE_BUILD'; 
		if(planned === false){
			title = 'New Coverage Build[Unplanned]';
			moduleType = 'MOD_COST_COVERAGE_UNPLANNED_BUILD';
			tableName = 'TB_STG_COST_COVERAGE_UNPLANNED_BUILD';
		}
		var reversalPanel = new DigiCompass.Web.app.ReversalPanel({
			...
			front : new DigiCompass.Web.app.ObjectGrid({
				...
				store : Ext.create('Ext.data.ArrayStore', {
					fields : [ 'id',  "version",  'siteType', 'capex', 'opex', 'income', 'expense',
				...
				columns : [{ 
						...
						renderer : function(value) {
							return !value.spectrumRegion ? null : value.spectrumRegion.name;
						}
					},{
					header : 'Site Type',
					sortable : false,
	...
	space.dragRefresh = function(arg, planned) {
		space.refresh(arg, planned);
	}
})();
