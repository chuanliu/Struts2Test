...
Ext.override(Ext.data.Store, {
	 ...
	 */
	countField:function(field , grouped){
	        var me = this;

	        if (grouped && me.isGrouped()) {
	            return me.aggregate(me.getSummaryCount, me, true, [field]);
	        } else {
	            return me.getSummaryCount(me.data.items, field);
	        }
	},
	getSummaryCount:function(records, field){
		 var i = 0,
         len = records.length,
         count = 0;
	     ...
	     if (records.length > 0) {
	         for (; i < len; ++i) {
	             if(!Ext.isEmpty(records[i].get(field))){
	            	 if(records[i].get(field)!= 0){
	            		 count++;
	            	 }
	             }
	         }
...
Ext.override(Ext.grid.feature.AbstractSummary, {
	 ...
	 */
	getSummaryField: function(store, type, field, group){
		if (type) {
			...
			switch (type) {
				case 'count':
					return store.countField(field , group);
				case 'min':
...
Ext.override(Ext.grid.feature.Summary, {
	...
	generateSummaryData: function(){
		...
		for (i = 0, length = columns.length; i < length; ++i) {
			...
			data[comp.dataIndex] = me.getSummaryField(store, comp.summaryType, comp.dataIndex, false);
		}
		return data;
	}
});
